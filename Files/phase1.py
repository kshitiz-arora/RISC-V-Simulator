...
def fetch():  # PC is of type string 0x0
    ...
    PC_temp = PC + 4
    message[0] = "FETCH:           PC_temp -> " + hex(PC+4) + "    Fetched instruction - " + instructions[PC]
...
def decode(instr):
    ...
    message[1] = "DECODE:          Intruction Type - " + instr_type + "    Operation - " + operation + "    Register values are read."
    return reg_list
...
def execute(reg_list):
    ...
    PC = PC_temp
    message[2] = "EXECUTE:         PC -> " + hex(PC) + "    Returned value - " + str(var)
    return var
...
def memoryAccess(var):
    ...
    if (instr_type == 'S'):
        message[3] = 'MEMORY ACCESS:   Memory at ' + hex(var) + ' is updated.'
        if (operation == 'sb'):
        ...
        message[3] = 'MEMORY ACCESS:   Data at ' + hex(var) + ' is accessed.'
        if (operation == 'lb'):
        ...
        message[3] = 'MEMORY ACCESS:   No Action'
...
def registerUpdate(var, memread):
    ...
    if (instr_type == 'R'):
        ...
        message[4] = 'REGISTER UPDATE: Register x' + str(reg) + ' is updated.'
    elif (instr_type == 'S'):
        message[4] = 'REGISTER UPDATE: No Action'
    elif (instr_type == 'I'):
        message[4] = 'REGISTER UPDATE: Register x' + str(reg) + ' is updated.'
        if (operation == 'jalr'):
            registers[reg] = '0x' + \
