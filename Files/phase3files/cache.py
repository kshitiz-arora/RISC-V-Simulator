...
def intialise_data_cache(cache_size, block_size, numSA):
    global data_cache
    global num_of_sets
...
def intialise_instr_cache(cache_size, block_size, numSA):
    global instr_cache
    global num_of_sets
    global num_of_blocks
    num_of_blocks = int(cache_size/block_size)
...
def readInstrCache(address): # type(address) = int
    ...
    if (isHit):
        ...
        instr_cache[numSet][index][3] = [memory[startAddress + i] for i in range(block_size)]
        reqMem = instr_cache[numSet][index][3][address-startAddress]
...
def readCache(address): # type(address) = int
    ...
    if (isHit):
        hits += 1 
        reqMem = data_cache[index][3][address-startAddress]
        ...
        if (isAvailable == False):
            for i in range(numSA):
                if (data_cache[i][1] == 0):
                    ...
                    index = i
                    break
        data_cache[index][0] = 1
        data_cache[index][1] = numSA-1
        data_cache[index][2] = startAddress
        data_cache[index][3] = [memory[startAddress + i] for i in range(block_size)]
        reqMem = data_cache[index][3][address-startAddress]
        
    for i in range(numSA):
        if (data_cache[i][0] == 1):
            data_cache[i][1] = max(data_cache[i][1]-1, 0)
    data_cache[index][1] = numSA-1
